...
let rec elim_induction_vars_loop (c : config) : config list =
  (* print_endline "elim_ind_vars_loop"; *)
  ...
  | e::est ->
     ...
     let res =
          ...
          (match e', vs with
           ...
           | If (bt, es1, es2), v :: vs' ->
              ...
              if (c.ct_check) then ( 
                (match res with
                 ...
                 | r::[] -> elim_induction_vars_loop r
                 | r1::r2::[] ->
                    if Z3_solver.is_ct_unsat (pclet, pc) v mem then
                      elim_induction_vars_loop r1 @ elim_induction_vars_loop r2
                    else (
              ...
              if (c.ct_check) then ( 
                (match res with
                 ...
                 | r1::r2::[] ->
                    ...
                    then elim_induction_vars_loop r1 @ elim_induction_vars_loop r2
                    else (
                      ConstantTime.warn e.at "BrIf: Constant-time Violation";
                      elim_induction_vars_loop r1 @ elim_induction_vars_loop r2)
                 | _ -> failwith "Infeasible Path"
                )
              )
              else (
                match res with
                 ...
                 * [{c with code = vs', es' @ List.tl es}] *)
                | r::[] -> elim_induction_vars_loop r
                | r1::r2::[] -> elim_induction_vars_loop r1 @ elim_induction_vars_loop r2
                | _ -> failwith "BrIf: Unreachable" 
              )
           (* Must be unsat *)
           ...
           | CallIndirect x, SI32 i :: vs ->
              ...
              (match i_sol with
               ...
               | _ -> let sols =
                      ...
                      in
                      List.fold_left (fun x y -> x @ elim_induction_vars_loop y) [] sols
              )
           ...
           | LocalSet x, v :: vs' ->
              ...
              (try
                 ...
                 let vs', es' = vs', [] in
                 elim_induction_vars_loop {c with code = vs', es' @ List.tl es; frame = frame'}
               with Not_found ->                 
                 let frame' = update_local c.frame x v in
                 let vs', es' = vs', [] in
                 elim_induction_vars_loop {c with code = vs', es' @ List.tl es; frame = frame'})
           | LocalTee x, v :: vs' ->
              print_endline "localtee_elim";
              let oldv = local frame x in
              let ivs = match c.induction_vars with Some ivs -> ivs | None -> failwith "No ivs"; in

              (try
                 print_endline "Trying";
                 print_endline (Pc_type.svalue_to_string oldv);
              ...
              if (c.ct_check) then (
                if (Z3_solver.is_v_ct_unsat (pclet, pc) final_addr mem) then
                  (
                    ...
                    in
                    elim_induction_vars_loop res)
                else (
                  ConstantTime.warn e.at "Load: Constant-time violation";
                  elim_induction_vars_loop {c with code = nv :: vs', [] @ List.tl es;
                                                   frame = frame;
                                                   pc = pclet, pc}
                )
           ...
           | Store {offset; ty; sz; _}, sv :: si :: vs' ->
              ...
              if (c.ct_check) then (
                if (Z3_solver.is_v_ct_unsat (pclet, pc) final_addr mems) then
                  ...
                  let nv = (try
                              print_endline "Trying";
                              print_endline (Pc_type.svalue_to_string lnv);
                  ...
                  in elim_induction_vars_loop res
                else (
                  ConstantTime.warn e.at "Store: Constant-time violation";
                  elim_induction_vars_loop {c with code = vs', [] @ List.tl es;
                                                   frame = frame;
                                                   pc = pclet, pc}
                )
              )
              else ( (* disable ct check *)
           ...
           | Convert cvtop, v :: vs' ->
              ...
              in elim_induction_vars_loop {c with code = vs', es' @ List.tl es}
       ...
       | Havoc lvs, vs ->
          failwith "Not allowed Havoc in elim_induction_vars_loop"
         
       | SecondPass  (n, es0, (vs', code')), vs ->
          failwith "Not allowed SecondPass in elim_induction_vars_loop"

       | FirstPass  (n, es0, (vs', code')), vs ->
          failwith "Not allowed FirstPass in elim_induction_vars_loop"
       ...
       | Frame (n, frame', code', pc', iv'), vs ->
          ...
          List.map (fun ci ->
              {c with code = vs, [Frame (n, ci.frame,
                                         ci.code, ci.pc, iv') @@ e.at] @ List.tl es;
                      (* pc = ci.pc; *)
                      observations = ci.observations;
                      induction_vars = ci.induction_vars;
                      (* frame = ci.frame *)
