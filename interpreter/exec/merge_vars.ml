  ...
  let rec merge_vars_i (lv: loopvar_t list)
    ...
    | (LocalVar (x,is_low,mo,v) as lvh) :: lvs ->
       ...
       (try
          ...
          (match old_val |> get_value_loopvar,v with
           | Some sv1, Some sv2 when sv1 = sv2->
                let mp' = LoopVarMap.add str (LocalVar (x,new_is_low,mo,Some sv1), num+1) mp in
                merge_vars_i lvs mp'
           | _,_ ->
    ...
    | (GlobalVar (x,is_low,mo,v) as lvh) :: lvs ->
       ...
       (try
          ...
          (match old_val |> get_value_loopvar,v with
           | Some sv1, Some sv2 when sv1 = sv2 ->
                let mp' = LoopVarMap.add str (GlobalVar (x,new_is_low,mo,Some sv1), num+1) mp in
                merge_vars_i lvs mp'
           | _,_ ->
