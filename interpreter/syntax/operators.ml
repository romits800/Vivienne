...
let s32_const n = Const (S32 n.it @@ n.at)
let s64_const n = Const (S64 n.it @@ n.at)
...
let s32_load align offset = Load {ty = S32Type; align; offset; sz = None}
let s64_load align offset = Load {ty = S64Type; align; offset; sz = None}
...
let s32_load8_s align offset =
  Load {ty = S32Type; align; offset; sz = Some (Mem8, SX)}
let s32_load8_u align offset =
  Load {ty = S32Type; align; offset; sz = Some (Mem8, ZX)}
let s32_load16_s align offset =
  Load {ty = S32Type; align; offset; sz = Some (Mem16, SX)}
let s32_load16_u align offset =
  Load {ty = S32Type; align; offset; sz = Some (Mem16, ZX)}
let s64_load8_s align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem8, SX)}
let s64_load8_u align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem8, ZX)}
let s64_load16_s align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem16, SX)}
let s64_load16_u align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem16, ZX)}
let s64_load32_s align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem32, SX)}
let s64_load32_u align offset =
  Load {ty = S64Type; align; offset; sz = Some (Mem32, ZX)}
...
let s32_store align offset = Store {ty = S32Type; align; offset; sz = None}
let s64_store align offset = Store {ty = S64Type; align; offset; sz = None}
...
let s32_store8 align offset =
  Store {ty = S32Type; align; offset; sz = Some Mem8}
let s32_store16 align offset =
  Store {ty = S32Type; align; offset; sz = Some Mem16}
let s64_store8 align offset =
  Store {ty = S64Type; align; offset; sz = Some Mem8}
let s64_store16 align offset =
  Store {ty = S64Type; align; offset; sz = Some Mem16}
let s64_store32 align offset =
  Store {ty = S64Type; align; offset; sz = Some Mem32}
...
let s32_clz = Unary (S32 S32Op.Clz)
let s32_ctz = Unary (S32 S32Op.Ctz)
let s32_popcnt = Unary (S32 S32Op.Popcnt)
let s64_clz = Unary (S64 S64Op.Clz)
let s64_ctz = Unary (S64 S64Op.Ctz)
let s64_popcnt = Unary (S64 S64Op.Popcnt)
...
let s32_add = Binary (S32 S32Op.Add)
let s32_sub = Binary (S32 S32Op.Sub)
let s32_mul = Binary (S32 S32Op.Mul)
let s32_and = Binary (S32 S32Op.And)
let s32_or = Binary (S32 S32Op.Or)
let s32_xor = Binary (S32 S32Op.Xor)
let s32_shl = Binary (S32 S32Op.Shl)
let s32_shr_s = Binary (S32 S32Op.ShrS)
let s32_shr_u = Binary (S32 S32Op.ShrU)
let s32_rotl = Binary (S32 S32Op.Rotl)
let s32_rotr = Binary (S32 S32Op.Rotr)
let s64_add = Binary (S64 S64Op.Add)
let s64_sub = Binary (S64 S64Op.Sub)
let s64_mul = Binary (S64 S64Op.Mul)
let s64_and = Binary (S64 S64Op.And)
let s64_or = Binary (S64 S64Op.Or)
let s64_xor = Binary (S64 S64Op.Xor)
let s64_shl = Binary (S64 S64Op.Shl)
let s64_shr_s = Binary (S64 S64Op.ShrS)
let s64_shr_u = Binary (S64 S64Op.ShrU)
let s64_rotl = Binary (S64 S64Op.Rotl)
let s64_rotr = Binary (S64 S64Op.Rotr)
...
let s32_eqz = Test (S32 S32Op.Eqz)
let s64_eqz = Test (S64 S64Op.Eqz)
...
let s32_eq = Compare (S32 S32Op.Eq)
let s32_ne = Compare (S32 S32Op.Ne)
let s32_lt_s = Compare (S32 S32Op.LtS)
let s32_lt_u = Compare (S32 S32Op.LtU)
let s32_le_s = Compare (S32 S32Op.LeS)
let s32_le_u = Compare (S32 S32Op.LeU)
let s32_gt_s = Compare (S32 S32Op.GtS)
let s32_gt_u = Compare (S32 S32Op.GtU)
let s32_ge_s = Compare (S32 S32Op.GeS)
let s32_ge_u = Compare (S32 S32Op.GeU)
let s64_eq = Compare (S64 S64Op.Eq)
let s64_ne = Compare (S64 S64Op.Ne)
let s64_lt_s = Compare (S64 S64Op.LtS)
let s64_lt_u = Compare (S64 S64Op.LtU)
let s64_le_s = Compare (S64 S64Op.LeS)
let s64_le_u = Compare (S64 S64Op.LeU)
let s64_gt_s = Compare (S64 S64Op.GtS)
let s64_gt_u = Compare (S64 S64Op.GtU)
let s64_ge_s = Compare (S64 S64Op.GeS)
let s64_ge_u = Compare (S64 S64Op.GeU)
...
let i32_declassify = Convert (I32 I32Op.Declassify)
...
let i64_declassify = Convert (I64 I64Op.Declassify)
